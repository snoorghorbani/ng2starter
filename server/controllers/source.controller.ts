...
const saveSourceData = (Result: any, timeOfReuquestSending: number, source: any) => {
  ...
  model
    .save()
    .then((Result: any) => { debugger; })
    .catch((Result: any) => { debugger; });
};
...
const scheduleForGetData = (() => {
  ...
  return (source: any) => {
    ...
    schedauledSources[source.Endpoint] = setInterval(() => {
      const now = Date.now();
      httpGet(source.Endpoint)
        .subscribe(
        response => saveSourceData(response, now, source),
        error => saveRequestFailure(error, now, source)
        );
    }, source.Interval);
  };
})();
