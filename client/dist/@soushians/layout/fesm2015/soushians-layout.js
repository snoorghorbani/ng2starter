...
class ToolbarMenuComponent {
     ...
     */
    _observe_on_layout_config_and_filter_routes() {
        this.menuItems$ = this.configurationService.config$.pipe(map(config => config.menuItems), combineLatest(this.user$), map(([routes, user]) => {
            if (!user.Roles)
                return [];
            if (user.Roles.length == 0) {
                return [];
